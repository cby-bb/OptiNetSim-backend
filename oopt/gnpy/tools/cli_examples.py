...
def transmission_main_example(args=None):
    ...
    _add_common_options(parser, network_default=_examples_dir / 'network.json')
    ...
    parser.add_argument('--spectrum', default=_examples_dir / 'initial_spectrum1.json', type=Path, help='user defined mixed rate spectrum JSON file')
    ...
    with open('oopt/gnpy/example-data/node.json','r',encoding='utf-8') as f:
        node = json.load(f)
    args.source = node['source']
    args.destination = node['destination']
    source = None
    if args.source:
        ...
        if len(powers_dbm) == 1:
            for elem in mypath:
                rl = {
                    ...
                    "gsnr_0.1nm": float,
                    "gsnr_signal": float,
                    "osnr_ase_0.1nm": float,
                    "osnr_ase_signal": float
                }
                if elem.uid == source.uid or elem.uid == destination.uid:
                    rl["uid"] = elem.uid
                    rl["gsnr_0.1nm"] = round(mean(elem.snr_01nm),2)
                    rl["gsnr_signal"] = round(mean(elem.snr),2)
                    rl["osnr_ase_0.1nm"] = round(mean(elem.osnr_ase_01nm),2)
                    rl["osnr_ase_signal"] = round(mean(elem.osnr_ase),2)
                    result['eq_result'].append(rl)
            a = round(mean(destination.snr_01nm),2)
            result['final_GSNR'] = a
            #print(mypath[-1])
    #print(result)
